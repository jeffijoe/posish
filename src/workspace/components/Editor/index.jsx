...
export const Editor = withTheme(
  observer(({ readOnly, value, onChange, theme, ...rest }) => {
    return (
      <div>
        <textarea
          className={css`
...
export const Highlighter = withTheme(
  observer(
    class Highlighter extends React.Component {
      onSelectionChange = e => {
        ...
        if (
          isChildOf(this._node, selection.anchorNode) &&
          isChildOf(this._node, selection.focusNode)
        ) {
          console.clear();
          console.log("yay", {
      ...
      renderFragment = (workspace, theme, fragment, depth = 1) => {
        ...
        if (!fragment.innerFragments || fragment.innerFragments.length === 0) {
          ...
          fragment.innerFragments.forEach((innerFrag, idx) => {
            if (idx === 0) {
              result.push(
                <span key={`${fragment.key}-part0`}>{fragment.parts[0]}</span>
              );
            }
            result.push(
              this.renderFragment(workspace, theme, innerFrag, depth + 1)
            );
            result.push(
              <span key={`${fragment.key}-part${idx + 1}`}>
                {fragment.parts[idx + 1]}
              </span>
            );
          });
        }
        ...
        return (
          <span className={spanClass} key={fragment.key} style={style}>
            ...
            {fragment.color && (
              <a
                onClick={e =>
                  e.preventDefault() || workspace.removeHighlight(fragment.key)
                }
                className={css`
                  ...
                  height: 16px;
                  cursor: pointer;
                  background-color: ${theme.id === "light"
                    ? "#f0f0fe"
                    : "#404049"};
                  z-index: ${depth};
                  border-radius: 50%;
          ...
          >
            {frags.length === 0 ? (
              ...
              <div
                ...
                `}
              >
                {frags.map(f => this.renderFragment(workspace, theme, f))}
              </div>
            )}
          </div>
        );
