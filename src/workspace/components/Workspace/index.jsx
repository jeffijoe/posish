...
const paneViews = {
  Template: observer(({ workspace }) => (
    <Editor
      onChange={value => workspace.set({ template: value })}
      value={workspace.template}
      placeholder="Write a template here"
    />
  )),
  Editor: observer(({ workspace }) => (
    <Editor
      onChange={value => workspace.set({ code: value })}
      value={workspace.code}
      placeholder="Write code to highlight here"
    />
  )),
  Highlighter: observer(({ workspace, workspaceStore }) => (
    <Highlighter workspace={workspace} workspaceStore={workspaceStore} />
  )),
  Output: observer(({ workspace }) => <Output workspace={workspace} />)
};
      ...
      <WorkspaceLayoutItem>
        <Footer>
          ...
          <a
            ...
            href="https://www.instagram.com/p/BYLNI6jDFri/"
          >
            cat on my shoulder
          </a>{" "}
          by{" "}
          ...
          <a
            target="_blank"
            rel="noopener noreferrer"
            href="https://github.com/jeffijoe/posish"
          >
            Github
          </a>.
        </Footer>
